# China DID Method Specification

## About

`China DID` fully adheres to the latest [DID Specification v1.0 from W3C](https://www.w3.org/TR/did-core/). It functions as the identity data carrier within the governable digital identity system and provides the foundational architecture for the digital trust service platform.

## Abstract

`China DID` has developed a distributed digital identity system based on blockchain technology, designed to offer decentralized, secure, autonomous, and privacy-protected on-chain identity services for individuals, enterprises, institutions, and digital assets. By providing trusted digital identities, it enables users to control their data while addressing challenges related to privacy, data protection, security, and governance, facilitating seamless data flow between entities.

## Status of This Document

This document is currently at version `v1.0.0` for `China DID` and will be updated in accordance with the [W3C DID Core Specification](https://www.w3.org/TR/did-core/). The latest version of the document is available at https://www.sheca.com/.

## China DID Method

The `China DID` identifier is defined as: ```china```. 

A DID using this method **MUST** begin with the prefix `did:china`. The remainder of the DID, following the prefix, is the namespace-specific identifier, as detailed below.

```
China-DID = "did:china:" china-specific-identifier
china-specific-identifier = 28*idchar
idchar = ALPHA / DIGIT
```

A valid `China DID` might be:

```
did:china:51uYnBT3KXnbHowvEn5ksG23quaJ
```

The steps to generate a `China DID` are as follows:

![](./img/gen-china-did-en.png)

## DID Document

The `China DID` document fully adheres to the [W3C DID Core Specification](https://www.w3.org/TR/did-core/) and includes several extensions based on the standard. The field definitions for the `China DID` document are as follows:
...
2. **`id`**: A required field that specifies the DID of the document. It must follow the `China DID` [naming rules](#China-DID-Method) and be formatted as a JSON string.
3. **`versionId`**: A required field that specifies the version of the `China DID` document, represented as a JSON string.
4. **`controller`**: An optional field that specifies one or more controllers of the `China DID` document (those authorized to modify the document). The format can be either a JSON string or a JSON array.
5. **`created`**: A required field that indicates the creation time of the `China DID` document.
6. **`updated`**: A required field that indicates the last update time of the `China DID` document.
7. **`deactivated`**: A required field that indicates the deactivation status of the `China DID` document.
...
An example of the DID document of ```did:china:51uYnBT3KXnbHowvEn5ksG23quaJ``` is as follows:

```json
{
	"context": ["https://www.w3.org/ns/did/v1"],
	"id": "did:china:51uYnBT3KXnbHowvEn5ksG23quaJ",
	"alsoKnownAs": [],
	"controller": ["did:china:51uYnBT3KXnbHowvEn5ksG23quaJ", "did:china:5LiUUEscVL5Lr3FNyEVw6mssCxVZ"],
	"verificationMethod": [{
		"id": "did:china:51uYnBT3KXnbHowvEn5ksG23quaJ#key-1",
		"controller": "did:china:51uYnBT3KXnbHowvEn5ksG23quaJ",
		"type": "SM3WithSM2",
	...
	"assertionMethod": ["did:china:51uYnBT3KXnbHowvEn5ksG23quaJ#key-1"],
	"service": [{
		"id": "did:china:51uYnBT3KXnbHowvEn5ksG23quaJ#test3333",
		"type": "1",
	...
	"proof": {
		"type": "SM3WithSM2",
		"created": "2024-09-25T21:40:59Z",
		"verificationMethod": "did:china:5LiUUEscVL5Lr3FNyEVw6mssCxVZ#key-1",
...
The creation, update, and deactivation of a `China DID` document can only be initiated by its controller. Therefore, the `controller` field within the document must list all controllers associated with the DID. By default, the DID Subject is the controller of the document. All write operations involving the document require authentication through a verification method.

### Create

The `China DID` system provides three main operations for DID creation:
...
| `-accountID`       | `string` | The `China DID` identifier.                                  |
| `-txID`            | `string` | The blockchain transaction ID.                               |
| `msg`              | `string` | A message providing additional information about the request. |

### Resolve

Related DID documents can be accessed and parsed via the `China DID` identifier.

#### Request Parameters

| **Parameter** Name | **Type** | Required | **Constraints**   | **Description**             |
| ------------------ | -------- | -------- | ----------------- | --------------------------- |
| `did`              | `string` | Yes      | `Max length: 255` | The `China DID` identifier. |
